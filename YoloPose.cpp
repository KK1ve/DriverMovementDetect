...
static void generate_proposals(
    ...
    std::vector<std::vector<Object_Pose>>& objects
)
{
    ...
    std::vector<int>& indices)
{
    ...
    while (true) {
        std::vector<std::vector<int>> private_del_list(bboxes.size());
...
#pragma omp parallel for
        for (int i = 0; i < bboxes.size(); i++) {
            ...
            if (iou >= nmsThreshold)
            {
                private_del_list[i].push_back(i);

            }
        }
        std::vector<int> del_list;
        for (int tid = 0; tid < bboxes.size(); tid++) {
            del_list.insert(del_list.end(), private_del_list[tid].begin(), private_del_list[tid].end());
        }
        for (int i = 0; i < del_list.size(); i++) {
            bboxes.erase(bboxes.begin() + del_list[i] - i);
        }

    }
}

static float intersection_area(const Object_Pose& a, const Object_Pose& b)
{
...
static void non_max_suppression(int num_cls, std::vector<std::vector<Object_Pose>>& proposals, std::vector<Object_Pose>& results,
...
int Yolo_Pose::load(const char* modeltype, int _target_size, const float* _mean_vals, const float* _norm_vals, std::vector<std::string> _class_name, bool use_gpu) {
...
int Yolo_Pose::detect(ncnn::Mat in_pad, std::vector<Object_Pose>& objects, std::vector<std::variant<float, int>> &result, float prob_threshold, float nms_threshold) {
    ...
    std::vector<std::vector<Object_Pose>> proposals(num_classes, std::vector<Object_Pose>(0));
...
int Yolo_Pose::draw(cv::Mat& rgb, std::vector<Object_Pose>& objects) {
    ...
    //for (Object_Pose& obj : objects) {
        Object_Pose& obj = objects[index];
        cv::rectangle(res, obj.rect, { 0, 0, 255 }, 2);
        char text[256];
        sprintf(text, "%s %.1f%%", class_names[obj.label].c_str(), obj.prob * 100);
        if (obj.label == 0) {
            sprintf(text, "%s pose: %s", text, obj.pose_name.c_str());
        }
        else {
            char face_text[256];
