...
void my_nms_boxes(std::vector<cv::Rect>& boxes, std::vector<float>& confidences, float nmsThreshold, std::vector<int>& indices)
{
    BBOX bbox;
...
static void non_max_suppression(std::vector<Object_pose>& proposals, std::vector<Object_pose>& results,
    ...
    float ratio_w = 1.0f, float conf_thres = 0.70f, float iou_thres = 0.65f)
