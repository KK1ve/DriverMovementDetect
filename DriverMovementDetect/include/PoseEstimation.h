...
{
    public:
        PE(const string& modelpath, float nms_thresh_ = 0.5, float conf_thresh_ = 0.5);
        void detect_multi_hot(const Mat& video_mat, vector<ObjectPose> &boxes);
        Mat vis_multi_hot(const Mat& frame, const vector<ObjectPose>& boxes,  bool show_action = false,  float action_thresh = 0.06);
    ...
    private:
        ...
        void generate_proposal_multi_hot(const float *pred, vector<ObjectPose> &boxes);
        Env env = Env(ORT_LOGGING_LEVEL_VERBOSE, "PE Video Pose Estimation");
