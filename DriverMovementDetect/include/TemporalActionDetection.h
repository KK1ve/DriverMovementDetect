...
class TAD
{
    public:
        ...
        vector<int> detect_one_hot(const Mat& video_mat, vector<Bbox> &boxes, vector<float> &det_conf, vector<vector<float>> &cls_conf);
    ...
    private:
        ...
        void generate_proposal_one_hot(int stride, const float *conf_pred, const float *cls_pred, const float *reg_pred, vector<Bbox> &boxes, vector<float> &det_conf, vector<vector<float>> &cls_conf);
        Env env = Env(ORT_LOGGING_LEVEL_VERBOSE, "TAD Video Action Detect");
