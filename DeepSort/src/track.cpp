...
Track::Track(KAL_MEAN & mean, KAL_COVA & covariance, int track_id, int n_init, int max_age, const FEATURE & feature)
{
    ...
    features = FEATURESS(1, Feature_Vector_Dim);
...
void Track::featuresAppendOne(const FEATURE & f)
{
    ...
    FEATURESS newfeatures = FEATURESS(size + 1, Feature_Vector_Dim);
    newfeatures.block(0, 0, size, Feature_Vector_Dim) = this->features;
